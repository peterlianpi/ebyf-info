...
const searchTalentMembers = async (
  query: string,
  year?: number,
): Promise<User[]> => {
...
export default function SearchBox({ year }: SearchBoxProps) {
  const searchFunction = async (query: string) => await searchTalentMembers(query, year);
  ...
  return (
    <EnhancedSearch
      ...
      emptyStateMessage={`Enter a name to check YF Talen ${year} participation`}
