...
This project uses `next-pwa` for PWA functionality, providing automatic service worker generation, caching, and offline support.

The PWA setup includes:

* Automatic service worker registration and caching via next-pwa plugin
* Offline fallback to `/~offline` page
* Comprehensive caching strategies for pages, assets, and API responses
* Web app manifest for home screen installation

## Current Setup

### next.config.ts
The PWA is configured using `@ducanh2912/next-pwa` with the following features:

- Service worker generated at `service-worker.js`
- Aggressive front-end navigation caching
- Stale-while-revalidate strategy for pages and assets
- Cache-first for images, network-first for API calls
- Offline fallback to `/~offline`

### Manifest
Located at `public/manifest.json` with icons in `public/icons/`

### Offline Page
Simple offline fallback at `app/~offline/page.js`

### Debug Component
`components/PWADebug.tsx` provides debug information for PWA status

## Migration from Manual PWA
Previously, this project had a manual PWA implementation with custom `public/sw.js` for push notifications. However, since switching to `next-pwa`, the custom SW has been removed as the plugin handles all service worker functionality automatically.

If push notifications are needed in the future, they can be implemented by extending the generated service worker or using additional plugins.
